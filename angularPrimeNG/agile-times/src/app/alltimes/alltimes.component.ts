...
import { ContextMenu, MenuItem, LazyLoadEvent, DataTable } from 'primeng/primeng';
...
export class AlltimesComponent implements OnInit {
  ...
  ngOnInit() {
    this.contextMenu = [
      { label: 'Debug', icon: 'fa fa-times', command: (event) => this.onDebug(this.seletedRows) },
      { label: 'Delete', icon: 'fa fa-close', command: (event) => this.onDelete(this.seletedRows) },
    ];
  }
  onDebug(selectedRows: any): any {
    console.log(selectedRows);

  }
  onDelete(seletedRows: any[]): any {
    console.log(seletedRows);
    this.allTimesheetData = this.allTimesheetData.filter((row) => {
      return !seletedRows.includes(row);
    });
  }

  selectedRow: any;
  seletedRows: Array<any>;
  ...
  onEditComplete(editInfo) {
    ...
    console.log(fieldChanged, newRowValues);
  }
  onRowSelect(rowInfo) {
  ...
  //Index DB accessed using dexie.js
